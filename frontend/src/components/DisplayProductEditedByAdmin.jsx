...
const DisplayProductEditedByAdmin = ({
    onClose,
    fetchData,
    getAllProduct
}) => {
    const [data,setData]=useState({
      ...fetchData,
        ...
        brandName:fetchData?.brandName,
        category:fetchData?.category,
        productImage:fetchData?.productImage || [],
        description:fetchData?.description,
        price:fetchData?.price,
        sellingPrice:fetchData?.sellingPrice
    })
    ...
    const handleSubmit=async(e)=>{
        ...
        const fetchResponce=await fetch(SummaryApi.updateProduct.url,{
                    ...
                    {data.productImage.map((e,index)=>{
                        return (
                            <div className=' relative flex justify-between bg-slate-100 group h-32 '>
                                <img src={e} alt={e}  className='bg-slate-100 border cursor-pointer object-contain h-32  ' 
                                onClick={()=>
...
export default DisplayProductEditedByAdmin